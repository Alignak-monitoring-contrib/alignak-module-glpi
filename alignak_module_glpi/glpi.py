...
def get_instance(plugin):
    ...
    :return:
    """
    # logger.info("[glpi] Get a glpidb data module for plugin %s" % plugin.get_name())
...
class Glpidb_broker(BaseModule):
    ...
    def __init__(self, mod_conf):
        ...
        self.database = getattr(mod_conf, 'database', 'glpidb')
        self.character_set = getattr(mod_conf, 'character_set', 'utf8')
        logger.info("[glpi] using '%s' database on %s:%d (user = %s)",
                    self.database, self.host, self.port, self.user)

        # Database tables update configuration
        self.update_availability = bool(getattr(mod_conf, 'update_availability', '0') == '1')
        self.update_shinken_state = bool(getattr(mod_conf, 'update_shinken_state', '0') == '1')
        ...
        logger.info("[glpi] updating availability: %s", self.update_availability)
        logger.info("[glpi] updating Shinken state: %s", self.update_shinken_state)
        logger.info("[glpi] updating services events: %s", self.update_services_events)
        logger.info("[glpi] updating hosts states: %s", self.update_hosts)
        logger.info("[glpi] updating services states: %s", self.update_services)
        logger.info("[glpi] updating acknowledges states: %s", self.update_acknowledges)
        ...
        logger.info('[glpi] periodical commit period: %ds', self.commit_period)
        logger.info('[glpi] periodical commit volume: %d lines', self.commit_volume)
        logger.info('[glpi] periodical DB connection test period: %ds', self.db_test_period)

    def init(self):
        logger.info("[glpi] initialized")
        return True
    ...
    def open(self):
        ...
        """
        try:
            logger.info("[glpi] Connecting to database %s (%s) ..." % (self.host, self.database))
            ...
            logger.info('[glpi] database connection established')
        except Exception as e:
            logger.error("[glpi] database connection error: %s", str(e))
            self.is_connected = False
    ...
    def close(self):
        self.is_connected = False
        logger.info('[glpi] database connection closed')
    ...
    def execute_query(self, query, do_debug=False):
        ...
        TODO: finish catch
        """
        logger.debug("[glpi] run query %s", query)
        try:
            self.db_cursor.execute(query)
            self.db.commit()
            return True
        except IntegrityError, exp:
            logger.warning("[glpi] A query raised an integrity error: %s, %s", query, exp)
            return False
        except ProgrammingError, exp:
            logger.warning("[glpi] A query raised a programming error: %s, %s", query, exp)
            return False
    ...
    def bulk_insert(self):
        ...
        """
        logger.info("[glpi] bulk insertion ... %d events in cache (max insertion is %d lines)", len(self.events_cache), self.commit_volume)

        if not self.events_cache:
            logger.info("[glpi] bulk insertion ... nothing to insert.")
            return

        if not self.is_connected:
            if not self.open():
                logger.warning("[glpi] database is not connected and connection failed")
                logger.warning("[glpi] %d events to insert in database", len(self.events_cache))
                return

        logger.info("[glpi] %d lines to insert in database (max insertion is %d lines)", len(self.events_cache), self.commit_volume)
        ...
        while True:
            try:
                ...
                logger.debug("[glpi] prepared all available events for commit")
                break
            except Exception, exp:
                logger.error("[glpi] exception: %s", str(exp))
        logger.info("[glpi] time to prepare %s events for commit (%2.4f)", events_to_commit-1, time.time() - now)
        logger.info("[glpi] query: %s", query)

        now = time.time()
        try:
            self.execute_query(query)
        except Exception as e:
            logger.error("[glpi] error '%s' when executing query: %s", e, query)
            self.close()
        logger.info("[glpi] time to insert %s line (%2.4f)", events_to_commit-1, time.time() - now)

    def manage_brok(self, b):
        """Got a brok, manage the interesting broks"""
        # Build initial host state cache
        if b.type == 'initial_host_status':
            host_name = b.data['host_name']
            logger.debug("[glpi] initial host status : %s", host_name)

            try:
                logger.debug("[glpi] initial host status : %s : %s", host_name, b.data['customs'])
                self.hosts_cache[host_name] = {
                    'hostsid': b.data['customs']['_HOSTID'],
                    'itemtype': b.data['customs']['_ITEMTYPE'],
                    'items_id': b.data['customs']['_ITEMSID']
                }
            except:
                self.hosts_cache[host_name] = {'items_id': None}
                logger.debug("[glpi] no custom _HOSTID and/or _ITEMTYPE and/or _ITEMSID for %s", host_name)

            logger.info("[glpi] initial host status : %s is %s", host_name, self.hosts_cache[host_name]['items_id'])
        ...
        if b.type == 'initial_service_status':
            ...
            logger.debug("[glpi] initial service status : %s", service_id)

            if not host_name in self.hosts_cache or self.hosts_cache[host_name]['items_id'] is None:
                logger.debug("[glpi] initial service status, host is not defined in Glpi : %s.", host_name)
                return

            try:
                logger.debug("[glpi] initial service status : %s : %s", service_id, b.data['customs'])
                ...
                logger.debug("[glpi] no custom _ITEMTYPE and/or _ITEMSID for %s", service_id)

            logger.info("[glpi] initial service status : %s is %s", service_id, self.services_cache[service_id]['items_id'])

        # Manage host check result if host is defined in Glpi DB
        if b.type == 'host_check_result':
            host_name = b.data['host_name']
            logger.debug("[glpi] host check result: %s: %s", host_name)
            ...
            if host_name in self.hosts_cache and self.hosts_cache[host_name]['items_id'] is not None:
                ...
                logger.debug("[glpi] host check result: %s, %d seconds", host_name, time.time() - start)
        ...
        if b.type == 'service_check_result':
            ...
            service_description = b.data['service_description']
            service_id = host_name+"/"+service_description
            logger.debug("[glpi] service check result: %s", service_id)
            ...
            if host_name in self.hosts_cache and self.hosts_cache[host_name]['items_id'] is not None:
                if service_id in self.services_cache and self.services_cache[service_id]['items_id'] is not None:
                    ...
                    logger.debug("[glpi] service check result: %s, %d seconds", service_id, time.time() - start)
    ...
    def record_host_check_result(self, b):
        ...
        logger.debug("[glpi] record host check result: %s: %s", host_name, b.data)
        ...
        if self.update_hosts:
            ...
            try:
                ...
                logger.error("[glpi] error '%s' when executing query: %s", exp, query)
        ...
        if self.update_acknowledges and b.data['state_id'] == 0:
            ...
            logger.debug("[glpi] acknowledge query: %s", query)
            try:
                self.execute_query(query)
            except Exception as exp:
                logger.error("[glpi] error '%s' when executing query: %s", exp, query)
    ...
    def record_service_check_result(self, b):
        ...
        logger.debug("[glpi] service check result: %s: %s", service_id, b.data)
        ...
        if self.update_services_events:
            logger.info("[glpi] append data to events_cache for service: %s", service_id)
            data = {}
    ...
    def record_shinken_state(self, hostname, service, b):
        ...
        logger.debug("[glpi] record shinken state: %s/%s: %s", hostname, service, b.data)
    ...
    def record_availability(self, hostname, service, b):
        ...
        # Insert/update in shinken state table
        logger.debug("[glpi] record availability: %s/%s: %s", hostname, service, b.data)
        # if hostname.startswith('sim'):
            # logger.warning("[glpi] record availability: %s/%s: %s", hostname, service, b.data)
        ...
        # if b.data['state_type_id']==0:
            # logger.warning("[glpi] record availability for: %s/%s, but no HARD state, ignoring ...", hostname, service)
        ...
        try:
            self.execute_query(query)
            res = self.fetchone()
            logger.debug("[glpi] record availability, select query result: %s", res)
        ...
        since_last_state = 0
        logger.debug("[glpi] current state: %s, last state: %s", current_state, last_state)
        ...
        if exists:
            ...
            logger.debug("[glpi] current data: %s", data)
            ...
            logger.debug("[glpi] record availability, update query: %s", query)
            try:
                self.execute_query(query)
            except Exception as exp:
                logger.error("[glpi] error '%s' when executing query: %s", exp, query)
        ...
        # Create record
            ...
            logger.debug("[glpi] record availability, insert query: %s", query)
            try:
                self.execute_query(query)
            except Exception as exp:
                logger.error("[glpi] error '%s' when executing query: %s", exp, query)
    ...
    def main(self):
        ...
        while not self.interrupted:
            logger.debug("[glpi] queue length: %s", self.to_q.qsize())
            start = time.time()

            # DB connection test ?
            if self.db_test_period and db_test_connection < start:
                logger.debug("[glpi] Testing database connection ...")
                # Test connection every N seconds ...
                db_test_connection = start + self.db_test_period
                if not self.is_connected:
                    logger.info("[glpi] Trying to connect database ...")
                    self.open()

            # Bulk insert
            if db_commit_next_time < start:
                logger.debug("[glpi] Logs commit time ...")
                # Commit periodically ...
            ...
            logger.debug("[glpi] time to manage %d broks (%d seconds)",
